...
public interface MeetRepository extends JpaRepository<Meet, UUID> {
    ...
    @Query(nativeQuery = true, value = "select * from meet where meet_status in ?1 and category_id in ?2 and latitude between ?3 and ?4 and longevity between ?5 and ?6")
    List<Meet> searchMeet(List<String> statuses, Set<UUID> categories, double lat_min, double lat_max, double lon_min, double lon_max);
